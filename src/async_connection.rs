...
use crate::{
    connection::{Connection, ToSql, SendPtr},
...
impl AsyncConnection {
    ...
    pub async fn query_with_parameters(&self, query: String, parameters: Vec<SqlParam>) -> Result<Option<Vec<Vec<String>>>, InformixError> {
        ...
        std::thread::spawn(move || {
            if let Ok(conn) = connection.lock() {
                ...
                let _ = tx.send(Err(InformixError::ConnectionError("Error getting connection lock".into())));
            }
        });
        match rx.await {
            Ok(result) => result,
            Err(e) => Err(InformixError::ConnectionError(format!("Error executing query: {}", e))),
        }
    }
}