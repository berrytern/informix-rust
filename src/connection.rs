...
impl Connection {
    pub fn new() -> Result<Self> {
        ...
        if result == 0 {
            ...
            if conn_result == 0 {
                Ok(Connection { handle: SendPtr::new(conn_handle), is_connected: AtomicBool::new(false) })
            } else {
    ...
    pub fn prepare(&self, sql: &str) -> Result<Statement> {
        ...
        let result = unsafe {
            SQLAllocHandle(SQL_HANDLE_STMT.into(), self.handle.as_ptr(), &mut stmt_handle)
        };
        if result != 0 {
    ...
    fn get_error_message(&self) -> String {
        ...
        unsafe {
            SQLGetDiagRec(SQL_HANDLE_DBC, self.handle.as_ptr(), 1, 
    ...
    pub fn execute(&self, sql: &str) -> Result<Statement> {
        let mut stmt_handle: *mut c_void = std::ptr::null_mut();
        let result = unsafe {
            SQLAllocHandle(3, self.handle.as_ptr(), &mut stmt_handle)
        };
        if result != 0 {
            return Err(InformixError::HandleAllocationError(result));
        }
